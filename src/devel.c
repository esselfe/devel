...
unsigned int winX = 200, winY = 10, winW = 1200, winH = 1000, winWorig = 1200, winHorig = 1000;
...
#ifdef WINDOWS
	if (verbose) {
		char *buffer = _getcwd (NULL, 1024);
		printf ("cwd:%s\n", buffer);
		free (buffer);
	}
#endif
    if (SDL_Init (SDL_INIT_VIDEO) < 0) {
        printf ("%s: SDL_Init() failed.\n", program_name);
        return 1;
    }
